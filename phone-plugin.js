...
// ========== 悬浮球 ==========
  function createFloatingBall() {
    // 【BUG 修复】：如果热重载导致之前有残留悬浮球，直接移除旧的并重新创建，确保 S.ball 永远不为空且事件绑定有效
    var existing = document.querySelector('.roche-phone-ball');
  ...
  async function captureScreenAndChat() {
    ...
    if (S.roche && S.roche.ui) S.roche.ui.toast('已发送屏幕画面，等待角色回复...');
    ...
    try {
       ...
       var res = await S.roche.ai.chat({
         messages: [
           ...
           { role: 'user', content: [
              { type: 'text', text: '这是我现在的屏幕，看看我在干嘛？' },
       ...
       if (res && res.text) {
         var msg = {
           ...
           text: res.text.trim(),
           timestamp: Date.now(),
           senderName: S.currentCharName
         };
         if (S.currentCharId) msg.senderId = S.currentCharId;
         await addPhoneMessage(msg);
         if (S.isOpen) renderMessages();
         else { S.unreadCount++; updateBadge(); }
       }
    } catch (e) {
  ...
  function createChatPanel() {
    var existing = document.querySelector('.roche-phone-panel');
    if (existing) existing.remove(); // 修复重载时可能存在的面板残留
  ...
  function buildPanelHTML() {
    ...
    return '<div class="roche-phone-header">'
      ...
      + '<button class="roche-phone-header-btn" id="roche-phone-mirror-btn" title="开启桌宠视角">' + MIRROR_SVG + '</button>'
      ...
      + '<div class="roche-phone-mirror-container" id="roche-phone-mirror-container"><video class="roche-phone-mirror-video" id="roche-phone-mirror-video" autoplay muted playsinline></video><button class="roche-phone-mirror-close" id="roche-phone-mirror-close" title="关闭视角">' + CLOSE_SVG + '</button></div>'
  ...
  function bindPanelEvents(panel) {
    ...
    panel.querySelector('#roche-phone-mirror-btn').addEventListener('click', function() { toggleMirror(); });
    panel.querySelector('#roche-phone-mirror-close').addEventListener('click', function() { stopMirror(); });
    // 双击头像切换角色
  ...
  async function toggleMirror() {
    ...
    try {
      ...
      S.mirrorStream = stream;
      var container = document.querySelector('#roche-phone-mirror-container');
      var video = document.querySelector('#roche-phone-mirror-video');
      if (container && video) {
      ...
      console.warn('[小手机] 无法获取屏幕流:', e);
      if (S.roche && S.roche.ui) S.roche.ui.toast("已取消或无法获取屏幕权限");
    }
  }

  function stopMirror() {
    if (S.mirrorStream) {
      S.mirrorStream.getTracks().forEach(function(track) { track.stop(); });
      S.mirrorStream = null;
    }
    var container = document.querySelector('#roche-phone-mirror-container');
    var video = document.querySelector('#roche-phone-mirror-video');
    ...
    apps: [{
      ...
      async mount(container, roche) {
        ...
        container.innerHTML = '<div class="roche-plugin-phone-app" style="padding:20px;font-family:-apple-system,BlinkMacSystemFont,sans-serif">'
          ...
          + '• 悬浮球可拖拽，位置自动保存<br>'
          + '• 头部小电视图标可开启【桌宠屏幕实时监控】<br>'
          + '• (新增) 双击悬浮球：自动截图当前网页给角色看<br>'
          + '• 双击头像切换角色<br>'
          + '• Enter 发送仅同步到线上<br>'
          + '• 发送键 同时触发线下回复</p>'
          + '</div>';
      ...
      async unmount(container) {
        ...
        S.intervals = []; stopCapture(); stopMirror(); window.removeEventListener('resize', onWindowResize); container.replaceChildren();
      }
    }]
  });
})();
